...
{     string Name { get; }
    ...
    //void Show(FormMain parent = null);
    //string ParseText(string Text);
    //void UpdatePanelBackgroundColor();
    //void VariableChanged(string var);
    //bool IsClosing { get; set; }
    //void EchoText(string sText, Color oColor, Color oBgColor);
}
...
//    private MapForm _m_Form;
    ...
    private string RoomOnDisk(Node oNode)
    {
        try
        {
            ...
            XmlNodeList xnlist;
            var diDirectory = new DirectoryInfo(m_oConfigSettings.MapDir);
            foreach (FileInfo dif in diDirectory.GetFiles())
            {
                ...
                {
                    TextFunctions.EchoText("[" + Name + "] Invalid maps in genie map directory: " + Path.GetFileName(dif.FullName) + " {" + ex.Message + "}");
                }
            }
        }
        catch
    ...
    private void EchoRoomsOnDisk(Node oNode)
    {
                ...
                foreach (XmlNode xn in xnlist)
                {
                    if ((oNode.Name ?? "") == (GetValue(xn, "name") ?? ""))
                    {
                        ...
                        if (bDescMatch == true)
                        {
                            // Check exits here in a later version
                            bMatch = true;
                            TextFunctions.EchoText("[" + Name + "] Found room #" + GetValue(xn, "id") + " on map: " + dif.FullName);
                        }
                    }
                }
            }
        ...
        if (bMatch == false)
        {
            TextFunctions.EchoText("[" + Name + "] No matches.");
        }
    }
